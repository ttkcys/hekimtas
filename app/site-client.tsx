  ...
  return (
    <div className="public-site" style={{ "--brand-primary": content.brand.primaryColor, "--brand-accent": content.brand.accentColor } as React.CSSProperties}>
      ...
      <footer className="new-footer">
        ...
        <div className="site-container new-footer__bottom"><span>© {new Date().getFullYear()} {content.brand.name}</span><div><a href="/hizmetler">Hizmetler</a><a href="/kategoriler">Kategoriler</a><a href="/blog">Blog</a>{content.socials.filter((item) => item.active && item.url).map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer">{item.platform}</a>)}<a href="/admin">Yönetim</a></div></div>
      </footer>
